...
import { RoomCardPropsType } from '@/interfaces';
import { useRouter } from 'next/navigation';

const RoomCard: React.FC<RoomCardPropsType> = ({
  id,
  slug,
  createdAt,
  user,
    ...
    <Card
      ...
      <div className='flex items-start space-x-4'>
        <div className='flex-shrink-0'>
          <Avatar size={48} src={user.photo} className='!bg-indigo-600'>
      ...
      <div className='mt-4 flex justify-between gap-2'>
        ...
        <Popover
          ...
          <button className='px-3 py-2 text-sm font-medium text-[#4F46E5] bg-indigo-50 rounded-md hover:bg-indigo-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500'>
            <Info size={18} />
          </button>
        </Popover>
      </div>
