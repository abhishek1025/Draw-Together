...
import { RoomFormFormikType } from "@/interfaces";

export default function AddRoomDialog() {
  const [open, setOpen] = useState<boolean>(false);
  const { roomFormLoading, roomFormError } = useAppSelector(
    (state) => state.room,
  );
  ...
  const formik = useFormik<RoomFormFormikType>({
    ...
    onSubmit: async (values: RoomFormFormikType) => {
      ...
      if (addNewRoom.rejected.match(result)) {
        toast.error(
          roomFormError ?? "Failed to create new room. Please try again later.",
        );
      }
        ...
        >
          <div className="grid w-full items-center gap-1.5">
            ...
            {formik.errors.slug && formik.touched.slug && (
              <div className="text-sm text-red-600">{formik.errors.slug}</div>
            )}
          </div>
