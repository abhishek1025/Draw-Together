...
const Chat = ({socket, roomId}: { socket: WebSocket, roomId: string }) => {
    ...
    const handleSendMessage = event => {
        ...
        if (message.type === 'chat_message') {
            setMessages((prevMessages) => [...prevMessages, {...message, user: JSON.parse(message.user)}]);
        }
    }
    ...
    const renderAvatar = (index: number, message: ChatMessage) => {
        ...
        if (index === 0 || (index > 0 && messages[index - 1]?.user.id !== message.user.id)) {
            return <Avatar
                size={30}
    ...
    return (
        <div className="fixed right-[20px] bottom-[30px] ">
            ...
            {displayChat && (<div className="w-[350px]">
                <div className="relative group">

                    <div className="absolute top-2 right-2">
                        <Button icon={<X/>} type="default" size="middle" className="rounded" onClick={closeChat}/>
                    </div>

                    <div
                        className="flex flex-col-reverse h-[400px] overflow-y-auto group-hover:bg-white group-hover:bg-opacity-10 mb-2 rounded">
                                ...
                                <div key={index} className="flex space-x-2">
                                    ...
                                    <div className="flex-1">
                                        <div className=" text-sm flex items-center flex-1">
                                            <p className="bg-white py-1 px-2 rounded inline">{message.message}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
                    ...
                    <div className="flex gap-x-5 items-center bg-white p-4 rounded">
                        <Input placeholder="Please enter your chat" value={newMessage} onChange={(e) => {
                            setNewMessage(e.target.value);
                        }}/>
                        <Button icon={<ArrowUp/>} type="primary" size="middle" className="" shape="circle"
                                onClick={sendMessage} disabled={!newMessage}/>
                    </div>
                </div>
